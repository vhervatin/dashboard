...
// API base URL
const API_BASE_URL = 'https://nwh.devautomatizadores.com.br/webhook/agenda';

// Fetch events with GET method
export async function fetchCalendarEvents(urls: any, selectedDate?: Date | null) {
  try {
    let url = urls.scheduleList;
    if (selectedDate) {
...
export async function refreshCalendarEventsPost(urls: any, selectedDate?: Date | null) {
  try {
    const payload: any = {};
    if (selectedDate) {
      const startDateTime = format(selectedDate, "yyyy-MM-dd'T'00:00:00.000xxx");
    ...
    const response = await fetch(urls.scheduleList, {
      method: 'POST',
...
// Add a new event
export async function addCalendarEvent(urls: any, formData: EventFormData) {
  try {
    ...
    const payload = { summary, description, start: startDateTime, end: endDateTime, email };
    console.log('Adding event with payload:', payload);
    const response = await fetch(urls.scheduleCreate, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
...
export async function editCalendarEvent(urls: any, eventId: string, formData: EventFormData) {
  try {
    const { date, startTime, endTime, summary, description, email } = formData;
    const dateStr = format(date, "yyyy-MM-dd");
    const startDateTime = `${dateStr}T${startTime}:00-03:00`;
    const endDateTime = `${dateStr}T${endTime}:00-03:00`;
    const payload = { id: eventId, summary, description, start: startDateTime, end: endDateTime, email };
    console.log('Updating event with payload:', payload);
    const response = await fetch(urls.scheduleUpdate, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
...
// Delete an event
export async function deleteCalendarEvent(urls: any, eventId: string) {
  try {
    const payload = { id: eventId };
    console.log('Deleting event with payload:', payload);
    const response = await fetch(urls.scheduleDelete, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
